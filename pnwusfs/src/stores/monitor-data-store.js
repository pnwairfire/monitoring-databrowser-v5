...
export const airsis = asyncReadable(
  ...
  async () => {
    ...
    // return monitor;
    // NOTE:  Custom filtering for the PNWUSFS site
    let ids = monitor.getIDs().filter((o) => o.indexOf("_pnwusfs") !== -1);
    let filtered_monitor = monitor.select(ids);
    return filtered_monitor;
  },
  { reloadable: true }
);

// All monitors combined (changes whenever any underlying data changes)
export const all_monitors = derived([airsis], ([$airsis]) => {
  let all_monitors = $airsis.dropEmpty();
