...
# calfire_cacher.py
#
# Fetch CalFire incidents GeoJSON, upload full and active-only versions to S3.
#
# - Source (default):
...
#
# Logging:
#   /app/logs/{process_name}_TRACE.log
#
...
def setup_logging(process_name: str, log_dir: str = "/app/logs") -> logging.Logger:
    """
    Configure logging to file and stdout.

    Log file pattern: /app/logs/{process_name}_TRACE.log
    """
...
def upload_json_to_s3(
    ...
    """
    Upload a JSON-serializable Python object to S3 as pretty-printed GeoJSON.
    """
    logger.info(f"Uploading to s3://{bucket}/{key}")
    tmp_file = None
    try:
        tmp_file = NamedTemporaryFile(mode="w", suffix=".geojson", delete=False)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Fetch CalFire GeoJSON and upload full/active subsets to S3."
    )
    parser.add_argument(
        "--source-url",
        default=os.getenv(
...
def main():
    ...
    try:
        ...
        # 2. Upload full incidents file
        key_all = f"{args.prefix}/incidents_all.geojson"
        upload_json_to_s3(s3_client, geojson_all, args.bucket, key_all, logger)

        # 3. Subset to active incidents
        geojson_active = subset_active_incidents(geojson_all, logger)

        # 4. Upload active incidents file
        key_active = f"{args.prefix}/incidents_active.geojson"
        upload_json_to_s3(s3_client, geojson_active, args.bucket, key_active, logger)
