...
export const airnow_geojson = asyncReadable(
  ...
  async () => {
    const response = await fetch(
      "https://airfire-data-exports.s3.us-west-2.amazonaws.com/ara-training-2025/sequence-b/monitoring/v2/latest/geojson/mv4_airnow_PM2.5_latest.geojson"
    );
    if (response.ok) {
...
export const airsis_geojson = asyncReadable(
  ...
  async () => {
    const response = await fetch(
      "https://airfire-data-exports.s3.us-west-2.amazonaws.com/ara-training-2025/sequence-b/monitoring/v2/latest/geojson/mv4_airsis_PM2.5_latest.geojson"
    );
    if (response.ok) {
      const userObject = await response.json();
...
export const wrcc_geojson = asyncReadable(
  ...
  async () => {
    const response = await fetch(
      "https://airfire-data-exports.s3.us-west-2.amazonaws.com/ara-training-2025/sequence-b/monitoring/v2/latest/geojson/mv4_wrcc_PM2.5_latest.geojson"
    );
    if (response.ok) {
      const userObject = await response.json();
...
export const airnow = asyncReadable(
  ...
  async () => {
    ...
    await monitor.loadLatest(
      "airnow",
      "https://airfire-data-exports.s3.us-west-2.amazonaws.com/ara-training-2025/sequence-b/monitoring/v2"
    );
    let end = Date.now();
